...
$jenis_barang_new=strtoupper($sub_jenis_barang); // $no_inventaris/$ID/$jenis_barang_new/$unit_new/$tahun_produksi

$sub_tgl_pengadaan=substr($tgl_pengadaan,5,2);
//menginput data ke database

$data=mysqli_query($koneksi, "select * from direksekutif where no_inventaris = '$no_inventaris/$ID/$jenis_barang_new/$unit_new/$sub_tgl_pengadaan/$tahun_operasional'") or die (mysqli_error());
$isi=mysqli_fetch_array($data);
...
mysqli_query($koneksi,"insert into direksekutif values('','$no_inventaris/$ID/$jenis_barang_new/$unit_new/$sub_tgl_pengadaan/$tahun_operasional','$unit','$tgl_pengadaan','$tahun_operasional','$jenis_barang','$merk','$lokasi','$kondisi','$harga','$status','$no_seri','$type' '')");
...
//header("location:../direksekutif.php");
