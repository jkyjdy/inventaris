...
//melakukan query dengan perintah UPDATE untuk update data ke database dengan kondisi WHERE dir_keuangan='$id' <- diambil dari inputan hidden id
$data=mysqli_query($koneksi, "select *from input_barang where nama='$jenis_barang'");
$isi=mysqli_fetch_array($data);
$ID1=$isi['ID']; 


//mysqli_query($koneksi,"insert into lain values('','$unit_new/$jenis_barang_new/$ID1/$tahun_produksi/$no_inventaris','$unit','$tgl_pengadaan','$tahun_produksi','$jenis_barang','$merk','$jumlah','$kondisi','$harga','$status')");
